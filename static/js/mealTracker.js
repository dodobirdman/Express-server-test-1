...
document.addEventListener('DOMContentLoaded', async function () {
    try {
        const response = await fetch('/meals'); // Fetch data from server endpoint
        if (!response.ok) {
            throw new Error('Failed to fetch meals');
        }
        const responseData = await response.json(); // Parse JSON response
        const mealsString = responseData[0].Meals; // Extract the "Meals" property
        const meals = JSON.parse(mealsString); // Parse the Meals string to get the array of meal objects

        // Now you can use the fetched meals data (meals) here
        localStorage.setItem('createdMeals', JSON.stringify(meals));
        // Retrieve meals from the meals array
